...
% -------- 环境搭建 --------
\textbf{H200 多机大模型 Post-training 训练链路搭建（Qwen3.5-27B / 397B）}
\begin{itemize}
    \item 基于 verl /Swift/ Megatron-LM / vLLM  搭建 H200 多机后训练环境，调试训练脚本，支持 Qwen3.5-27B 的 SFT、GRPO/DAPO 训练与 Qwen3.5-397B 全量 SFT 链路验证；
...
\end{itemize}
\textbf{{SFT 医学数据全量微调与分析（Qwen3.5-397B）}}
\begin{itemize}
    \item 基于 Swift 框架（Megatron后端）完成 Qwen3.5-27B 全量 SFT 训练（3 万条体检报告单数据）；
\end{itemize}
