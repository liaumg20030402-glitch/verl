...
class AdvantageEstimator(str, Enum):
    ...
    GDPO = "gdpo"
    MAXRL = "maxrl"  # ★ MaxRL（Maximum Likelihood RL, arXiv:2602.02710）：GRPO 同款，分母 std→组均值 r̂
...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        # ★ genrm 软丢弃：valid_mask[i]=False 的样本（如 GenRM 判分超时/失败）既**不进组 mean/std 统计**、
        #   优势也**直接置 0**（对 policy 梯度零贡献）。valid_mask=None 时全部有效（原始行为）。
        if valid_mask is None:
            valid = [True] * bsz
        else:
            valid = [bool(v) for v in valid_mask.reshape(-1).tolist()]
        for i in range(bsz):
            if valid[i]:
        ...
        for i in range(bsz):
            if not valid[i] or index[i] not in id2mean:
                # 失败样本（或整组都失败）→ 优势置 0
                scores[i] = 0.0
...
def compute_maxrl_outcome_advantage(
    ...
    """MaxRL（Maximum Likelihood RL，arXiv:2602.02710）的 outcome 优势。

    与 GRPO 唯一的区别是**分母**（论文说的“只改一行优势计算”）：
        GRPO :  A_i = (r_i - r̂) / (std + eps)
        MaxRL:  A_i = (r_i - r̂) / (r̂ + eps)            ← 只把分母 std 换成组均值 r̂
    其中 r̂ = id2mean = 组内平均奖励（**二元 0/1 时即“通过率 p”**）。

    关于论文里的 N：论文写成 A_i = (r_i - r̂)/(N r̂)，那个 1/N 是“对 mini-batch 求和再归一”的
    系数；而 verl 的 policy loss 本来就对样本取**均值**(= 自带 1/N)，所以这里**不再显式乘 N**，
    与 GRPO 优势 (r-μ)/σ 不含 N 的写法保持一致。如此 MaxRL 与 GRPO 的优势量级在 p≈0.5 时相当
    （都 ≈1），同一 lr 即可公平对照；难题(p 小)时 1/r̂ 让其优势更大（~1/p，比 GRPO 的 ~1/√p 更猛）。

    直觉：按 1/r̂ 放大学习信号 → **通过率越低（越难）的题权重越大**，逼近“对正确轨迹做最大似然”，
    缓解 GRPO 用 1/std 归一带来的“偏好简单题 + 分布锐化”。其余（按 uid 分组、广播到 token、
    genrm 软丢弃 valid_mask）与 GRPO 完全一致。

    ⚠️ 适用前提：奖励必须是**二元 0/1**（论文 r_i∈{0,1}）。叠加 overlong 长度惩罚后 r_i 会变成
       非 0/1、r̂ 可能≈0 或为负 → 1/r̂ 失去“似然权重”意义。所以用 MaxRL 时请**关掉 overlong 惩罚、
       保持纯 0/1 奖励**。下面对 r̂≤0 的组直接置 0（这些组本就该被 filter_groups 滤掉）。
    """
    scores = token_level_rewards.sum(dim=-1)
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        # genrm 软丢弃：valid_mask[i]=False 的样本不进组统计、优势置 0（与 GRPO 一致）
        if valid_mask is None:
            valid = [True] * bsz
        else:
            valid = [bool(v) for v in valid_mask.reshape(-1).tolist()]
        ...
        for idx in id2score:
            scores_tensor = torch.stack(id2score[idx])
            id2mean[idx] = torch.mean(scores_tensor)   # r̂ = 组均值（二元 0/1 时即通过率 p）
        for i in range(bsz):
            if not valid[i] or index[i] not in id2mean:
                scores[i] = 0.0
            else:
                mean = id2mean[index[i]]               # r̂
                if mean <= epsilon:
                    # 组均值≈0（全错）→ 无似然信号，优势置 0（这些组本就该被 filter_groups 滤掉）
                    scores[i] = 0.0
                else:
