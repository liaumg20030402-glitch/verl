...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # ★ genrm 软丢弃：失败/超时样本(genrm_failed==1)排除出组 mean/std 统计 + 优势置 0。
        #   标记来自远程 GenRM 奖励函数 reward_extra_info["genrm_failed"]；其它模式无此键则 None。
        valid_mask = None
        if "genrm_failed" in data.non_tensor_batch:
            valid_mask = (
                torch.as_tensor(np.asarray(data.non_tensor_batch["genrm_failed"], dtype=np.float32)) <= 0.5
            )
        ...
        # ★ MaxRL（arXiv:2602.02710）：和 GRPO 同样的分组+广播，只是分母 std→N·mean（见
        #   core_algos_my.compute_maxrl_outcome_advantage）。这里复用 GRPO 那套 genrm 软丢弃。
        valid_mask = None
        if "genrm_failed" in data.non_tensor_batch:
            valid_mask = (
                torch.as_tensor(np.asarray(data.non_tensor_batch["genrm_failed"], dtype=np.float32)) <= 0.5
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # ★ genrm 软丢弃：失败/超时样本已在 compute_advantage 内处理
                        #   （排除组 mean/std 统计 + 优势置 0），这里只统计数量供监控。
                        if "genrm_failed" in batch.non_tensor_batch:
                            metrics["reward/genrm_failed"] = float(
                                np.asarray(batch.non_tensor_batch["genrm_failed"], dtype=np.float32).sum()
                            )
